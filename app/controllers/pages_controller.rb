class PagesController < ApplicationController
  ...
  def user
    ...
    @user = @users.find(current_user.id)
    @mates = []
    @favorite_users = Favorite.where(user_id: @user.id).reject { |u| u.favorite_user_id.nil? }.map(&:favorite_user)
    unless params[:name] == ''
  ...
  def community
    ...
    @favorite_communities = Favorite.where(user_id: @user.id).reject { |u| u.community_id.nil? }.map(&:community)
    unless params[:name] == ''
